...
<main class="mx-auto max-w-[1600px] px-4 py-10 sm:px-6 lg:px-8">
  <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
    ...
    <div class="w-full max-w-md">
      <label for="docs-search" class="mb-2 block text-xs uppercase tracking-[0.18em] text-[var(--color-text-muted)]">Search docs</label>
      <input
        id="docs-search"
        bind:value={query}
        type="search"
      ...
      {#if query.trim()}
        <div class="panel-soft mt-3 rounded-[var(--radius-lg)] p-3">
          <div class="max-h-64 overflow-y-auto">
            {#each searchIndex.filter((entry) => `${entry.title} ${entry.description} ${entry.tokens.join(" ")}`.toLowerCase().includes(query.toLowerCase())).slice(0, 8) as entry}
              <a href={entry.href} class="block rounded-xl px-3 py-2 transition hover:bg-white/4">
                <div class="text-sm font-medium text-white">{entry.title}</div>
                <div class="text-xs text-[var(--color-text-muted)]">{entry.section}</div>
              </a>
            {/each}
          </div>
        </div>
  ...
  <div class="grid gap-8 xl:grid-cols-[280px_minmax(0,1fr)] 2xl:grid-cols-[280px_minmax(0,1fr)_240px]">
    <DocsSidebar groups={docsBySection} currentPath={`/${data.doc.slug.join("/")}`} search={query} />
    <DocsArticle
      page={data.doc}
      html={data.html}
      previousHref={data.previous ? `/${data.previous.slug.join("/")}` : undefined}
