<script lang="ts">
  ...
  let { cards } = $props<{
    cards: Array<{ label: string; description: string; href: string }>;
  }>();
</script>
...
<main class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
  ...
  <section class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
    {#each cards as card}
      <a
        href={card.href}
        class="panel group flex h-full flex-col rounded-[var(--radius-lg)] p-6 transition hover:-translate-y-1 hover:border-[var(--color-primary)]/30 hover:shadow-[0_20px_60px_rgba(132,204,22,0.08)]"
      >
        <div
          class="mb-4 inline-flex rounded-full border border-white/8 bg-white/4 px-3 py-1 text-xs uppercase tracking-[0.2em] text-[var(--color-text-muted)]"
        >
          {card.label}
        </div>
        ...
        <div
          class="mt-6 inline-flex items-center gap-2 text-sm font-medium text-[var(--color-primary-light)]"
        >
          <span>Open section</span>
          <svg
            aria-hidden="true"
            class="arrow-icon h-4 w-4"
            viewBox="0 0 20 20"
            fill="none"
          >
            <path
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <div>
        ...
        <p class="text-sm leading-7 text-[var(--color-text-muted)]">
          Install, first program, build modes, then jump into language and
          toolchains. The docs are organized to take you from first run to real
          native builds without dead ends.
        </p>
      </div>
      <div>
        <h2 class="mb-3 text-xl font-semibold text-white">
        ...
        <p class="text-sm leading-7 text-[var(--color-text-muted)]">
          Every docs page has a local search field. It is lightweight by design
          and works from the same content manifest that drives the sidebar and
          prev-next links.
        </p>
      </div>
      <div>
        ...
        <p class="text-sm leading-7 text-[var(--color-text-muted)]">
          The docs track the same builtin, decorator, and stdlib metadata that
          powers the formatter and LSP, so this content is meant to stay aligned
          with the product instead of drifting.
        </p>
      </div>
    </div>
  </section>
