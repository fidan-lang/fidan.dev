...
import { detectHost, docsCanonical, isLocalHost, mainCanonical } from "$lib/utils/hosts";
...
export const load: LayoutServerLoad = ({ url, request }) => {
  ...
  if (!isLocalHost(hostname) && host === "main" && pathname.startsWith("/docs")) {
  ...
  const canonicalUrl =
    host === "docs" || pathname.startsWith("/docs")
      ? docsCanonical(pathname === "/docs" ? "/" : pathname.replace(/^\/docs/, "") || "/", search)
