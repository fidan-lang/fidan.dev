...
It is not a promise that the language will stop evolving. It is a promise that the foundation is solid enough to build on.`,
  ...
  },
  {
    ...
    installNotes: ["Upgrade to 1.0.0 unless you specifically need to compare pre-stable behavior."],
...
export const latestRelease = releases.find((release) => release.latest) ?? releases[0];
...
for (const release of releases) {
  if (!release.version || !release.title || !release.summary) {
    throw new Error(`Invalid release metadata for version ${release.version || "<missing>"}`);
  }
}
